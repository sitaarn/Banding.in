...
class AuthController
{
     ...
     * Autentikasi Seller
     */
    public function authenticateSeller()
    {
        $username = sanitize($_POST['username'] ?? '');
        $password = $_POST['password'] ?? '';
        ...
        if ($validator->isValid()) {
            ...
            if ($user && ($user['role'] ?? '') === 'seller') {
                ...
                $_SESSION['errors_messages'] = 'Username/password salah atau akun bukan seller.';
                redirect(\BASE_URL . 'login?mode=seller');
            }
        } else {
            $_SESSION['errors_messages'] = $validator->getFirstError();
            redirect(\BASE_URL . 'login?mode=seller');
     ...
     * Register Seller
     */
    public function storeSellerUser()
    {
        $errors = [];
        $old = [
            'username' => sanitize($_POST['username'] ?? ''),
        ...
        $validator->required('username', 'Username wajib diisi.')
            ...
            ->required('nama_lengkap', 'Nama toko wajib diisi.')
            ->minLength('nama_lengkap', 3, 'Nama toko minimal 3 karakter.')
            ->required('password', 'Password wajib diisi.')
        ...
        if ($validator->isValid()) {
            ...
            if (empty($errors)) {
                ...
                if ($userId) {
                    setFlashMessage('success', 'Registrasi seller berhasil! Silakan login.');
                    redirect(\BASE_URL . 'login?mode=seller');
                } else {
